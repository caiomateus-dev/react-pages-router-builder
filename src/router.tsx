...
export function convertPagesToRoute(
  ...
  // Sort files to process parent routes before child routes
  const sortedFiles = Object.entries(files).sort(([a], [b]) => {
    const aSegments = a.split("/").length;
    const bSegments = b.split("/").length;
    return aSegments - bSegments; // Process shorter paths first
  });

  sortedFiles.forEach(([filePath, importer]) => {
  ...
  // Flatten nested page routes to make them siblings
  flattenPageRoutes(routes);
...
function mergeRoutes(
  ...
  // If target doesn't have element but source does, use source's element
  ...
  if (source.children) {
    ...
    source.children.forEach((sourceChild) => {
      ...
      if (matchingChild) {
        ...
        // Always add as child, but we'll handle the hierarchy differently
...
/**
 * Flattens nested page routes to make them siblings
 */
function flattenPageRoutes(routes: ExtendedRouteObject) {
  if (!routes.children) return;

  routes.children.forEach((child) => {
    if (
      child.handle?.pageType === "page" &&
      child.children &&
      ...
      childrenToMove.forEach((grandChild) => {
        if (grandChild.handle?.pageType === "page") {
          // Create a flat path for the grandchild
          const flatPath = `${child.path}/${grandChild.path}`;
          grandChild.path = flatPath;

          // Add to parent's parent (making it a sibling)
          routes.children?.push(grandChild);
        }
      });
    }

    // Recursively process children
    flattenPageRoutes(child);
  });
}
...
function createRoute(args: {
  ...
  // Always attach loading state for layouts and pages
  if (
